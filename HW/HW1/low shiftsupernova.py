...
qq=0.1
logz_data = [math.log10(float(datum[1])) for datum in data if float(datum[1])>qq ]
mu_data = [float(datum[2]) for datum in data if float(datum[1])>qq ]
mu_err_data = [float(datum[3]) for datum in data if float(datum[1])>qq]
